...
export function useVoiceInput(onSuccess: (result: VoiceTaskResponse) => void) {
  ...
  const start = async () => {
    ...
    if (!permission.granted) {
      setError(
        'Microphone access is required to add tasks by voice. Enable it in your device settings and try again.'
      );
      setStatus('error');
      return;
    }
  ...
  const stop = async () => {
    try {
      ...
      const messages: Record<string, string> = {
        CONFIGURATION:
          'Voice service is not configured. Add EXPO_PUBLIC_VOICE_API_URL and restart Expo.',
      ...
      setError(
        messages[code] ??
          'We could not process this recording. Please try again or add the task manually.'
      );
      setStatus('error');
    }
  };
