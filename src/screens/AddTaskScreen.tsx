    ...
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.root} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.field}>
          ...
          <View style={styles.quickDates}>
            {quickDates.map(item => {
              ...
              return (
                ...
                >
                  <Text style={[styles.dateChipText, selected && styles.dateChipTextSelected]}>{item.label}</Text>
                </Pressable>
              );
            })}
          </View>
          ...
          >
            <View>
              <Text style={styles.dateValue}>{dueDate ? formatDueDate(dueDate) : 'Choose a date'}</Text>
              <Text style={styles.dateHint}>{dueDate ? 'Tap to choose another date' : 'No deadline selected'}</Text>
            </View>
          ...
          {!!dueDate && (
            <Pressable accessibilityRole="button" onPress={() => setDueDate(undefined)} style={styles.clearDate}>
...
const createStyles = (theme: AppTheme) => StyleSheet.create({
  ...
  dateChipSelected: { backgroundColor: theme.colors.primarySoft, borderColor: theme.colors.primary },
  ...
  pickerWrap: { backgroundColor: theme.colors.surface, borderRadius: theme.radius.md, marginTop: 10, overflow: 'hidden' },
  ...
  pressed: { opacity: .86 },
  buttonText: { color: theme.dark ? theme.colors.background : theme.colors.white, fontWeight: '900', fontSize: 16 }
});
