...
export type TaskAction =
  ...
  | {
      type: 'UPDATE_TASK';
      id: string;
      changes: Pick<Task, 'title'> & Partial<Pick<Task, 'description' | 'dueDate'>>;
      at: string;
    }
...
export function taskReducer(state: TaskState, action: TaskAction): TaskState {
  switch (action.type) {
    case 'LOAD_TASKS':
      return { ...state, tasks: action.tasks, isHydrated: true };
    case 'ADD_TASKS':
    ...
    case 'UPDATE_TASK':
      return {
        ...
        tasks: state.tasks.map(task => {
          ...
          // Optional fields are assigned even when undefined so users can clear them.
          return {
            ...task,
    ...
    case 'TOGGLE_TASK':
      return {
        ...
        tasks: state.tasks.map(task =>
          task.id === action.id
            ? { ...task, completed: !task.completed, updatedAt: action.at }
            : task
        )
      };
    case 'DELETE_TASK':
    ...
    case 'STORAGE_ERROR':
      return { ...state, storageError: action.message, isHydrated: true };
    default:
      return state;
  }
}
