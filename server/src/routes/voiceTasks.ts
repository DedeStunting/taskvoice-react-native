...
export function voiceTasksRouter(client: OpenAI) {
  ...
  router.post('/', upload.single('audio'), async (request, response) => {
    if (!request.file) { response.status(400).json({ error: 'An audio file is required.' }); return; }
    if (!request.file.mimetype.startsWith('audio/')) { response.status(415).json({ error: 'The uploaded file must be audio.' }); return; }
    try {
      const transcript = await transcribeAudio(client, request.file.buffer, request.file.originalname, request.file.mimetype);
      if (!transcript) { response.status(422).json({ error: 'No speech was detected.' }); return; }
      try {
        const tasks = await extractTasks(client, transcript);
        response.json(tasks.length ? { transcript, tasks } : { transcript, tasks: [{ title: transcript }], fallback: true });
      } catch {
